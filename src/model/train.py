...
def train_model(train_data, val_data, trial_or_params):
    ...
    if isinstance(trial_or_params, dict):
        ...
        learning_rate = trial_or_params.suggest_float('learning_rate', 1e-4, 1e-1, log=True)
    ...
    with mlflow.start_run():
        ...
        history = model.fit(
            ...
            batch_size=batch_size,
            epochs=epochs,
            callbacks=[KerasPruningCallback(trial_or_params, 'val_accuracy')] if not isinstance(trial_or_params, dict) else []
        )
